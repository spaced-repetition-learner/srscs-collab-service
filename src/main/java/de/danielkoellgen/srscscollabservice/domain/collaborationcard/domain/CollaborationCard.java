...
public class CollaborationCard {
    ...
    public static @NotNull Pair<CollaborationCard, List<Correlation>> createNew(
            @NotNull Collaboration collaboration, @NotNull UUID rootCardId,
            @NotNull UUID cardOwnerUserId, @NotNull UUID cardOwnerDeckId) {
        ...
        List<Correlation> pendingCorrelations = collaboration.getParticipants().values()
                .stream()
                ...
                .toList();
        logger.debug("Created {} Correlations for other Participants which are active and have a Deck.",
                pendingCorrelations.size());
